...
import { Trash2, Plus, Image as ImageIcon, Loader } from 'lucide-react';
...
const Gallery: React.FC<GalleryProps> = ({ isAdmin }) => {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('photos').insert([newItem]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {photos.length === 0 && (
            <div className="col-span-full flex flex-col items-center justify-center py-20 text-gray-500">
              ...
              <p className="text-xl">Aucune photo dans le système.</p>
            </div>
          )}
        </div>
      )}
